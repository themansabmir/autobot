...
const sendWhatsAppMessage = async (
  ...
  // To track when a recipient starts/completes the flow without breaking core logic,
  // we capture the 'resultId' generated by the startSession call. This resultId is
  // permanently linked to all user answers (AnswerV2) and flow outcomes (Result).
  // Our background 'analyticsTracker' then polls these tables via this ID.
  const generatedResultId = startResponse.resultId ?? startResponse.newSessionState.typebotsQueue[0]?.resultId;
  if (generatedResultId) {
    await prisma.campaignRecipient.update({
      where: { id: recipientId },
      data: { resultId: generatedResultId },
    });
    console.log(`🔗 CampaignRecipient ${recipientId} linked to result ${generatedResultId}`);
  }
...
const main = async () => {
  ...
  // Start the decoupled analytics tracker
  // This tracker queries the Result and AnswerV2 tables for the linked resultIds
  // to populate CampaignRecipient (startedAt, completedAt, npsScore) asynchronously.
  runAnalyticsTracker();
